...
1) 准备权重
- 基座模型：放到 `weights/base/llava-v1.5-7b`，或设置 `X2DFD_BASE_MODEL=/path/to/llava-v1.5-7b`。
- 如使用独立视觉塔：放到 `weights/base/clip-vit-large-patch14-336`（或通过 `VISION_TOWER` 指定）。
...
- 快速推理：
  `python -m eval.infer.runner --config eval/configs/infer_config.yaml --json datasets/raw/data/Deepfakes_small.json`
